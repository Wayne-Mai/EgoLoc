...
class BoundingBox():
    ...
    # note : the annotation coordinate system should rotate 90 degree to match with matterport system
    def bounds(self):
        # we should return: (xMin, xMax, yMin, yMax, zMin, zMax)
        my_box=self.build_box()
        xMin=np.min([x[0] for x in my_box])
        xMax=np.max([x[0] for x in my_box])
        yMin=np.min([x[1] for x in my_box])
        yMax=np.max([x[1] for x in my_box])
        zMin=np.min([x[2] for x in my_box])
        zMax=np.max([x[2] for x in my_box])
    ...
    def mp_center(self):
        ...
        res=[]
        for i in range(3):
            res.append((my_bounds[2*i]+my_bounds[2*i+1])/2)
        return np.array(res)
